    ...
    <form action="{{ route('compounds.update', $compound->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="upload-section">
            <div class="upload-area">
                <input type="file" id="imageUpload" name="image" style="display: none;" accept="image/*" onchange="handleFileSelect(); previewImage(event)">
                <img id="imagePreview" src="{{ asset('storage/' . $compound->image) }}" alt="Current Image" style="max-width: 100%; max-height: 300px; display: {{ $compound->image ? 'block' : 'none' }};">
                <p style="display: {{ $compound->image ? 'none' : 'block' }};">Click here to upload your files or drag.</p>
                <p class="text-muted">Supported formats: SVG, JPG, PNG (1200 x 800)</p>
                <button type="button" onclick="document.getElementById('imageUpload').click();">Update Image</button>
                <div id="uploadMessage" style="display: none;">Image updated successfully!</div>
                @error('image')
            ...
            <div class="extra-fields">
                <div class="form-group">
                    <label for="fa1">Compound Name</label>
                    <input type="text" id="field1" name="name" value="{{ old('name', $compound->name) }}" placeholder="Compound Name">
                    @error('name')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="input1" name="developer_id" class="form-select">
                        @foreach($developers as $dev)
                            <option value="{{ $dev->id }}" {{ $compound->developer_id == $dev->id ? 'selected' : '' }}>
                                {{ $dev->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" style="grid-column: span 2;">
                    <label for="field3">Location</label>
                    <select name="location" id="field3" class="form-select">
                        @foreach(['6 October City', 'New Cairo', 'New Capital City', 'El Sheikh Zayed', 'Mostakbal City', 'Ain Sokhna', 'El Gouna', 'New Heliopolis', 'El Shorouk'] as $location)
                            <option value="{{ $location }}" {{ $compound->location == $location ? 'selected' : '' }}>
                                {{ $location }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="section1">
                <div class="form-row">
                    <div class="form-group first-group">
                        ...
                        <input type="number" name="starting_price" value="{{ old('starting_price', $compound->starting_price) }}" id="No">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="down_payment" value="{{ old('down_payment', $compound->down_payment) }}" id="No">
                        @error('down_payment')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="installment_options" value="{{ old('installment_options', $compound->installment_options) }}" id="No">
                    ...
                    <div class="form-group">
                        <label for="No">Maintenance (%)</label>
                        <input type="number" name="maintenance_deposit_percentage" value="{{ old('maintenance_deposit_percentage', $compound->maintenance_deposit_percentage) }}" id="No">
        ...
        <div class="form-section">
            ...
            <div class="section1">
                <div class="form-row">
                    <div class="form-group">
                        <label for="No">Unit Area (From)</label>
                        <input type="number" name="min_size" value="{{ old('min_size', $compound->min_size) }}" id="No">
                        @error('min_size')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label for="No">Unit Area (To)</label>
                        <input type="number" name="unit_area_to" value="{{ old('unit_area_to', $compound->unit_area_to) }}" id="No">
                        @error('unit_area_to')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label for="No">Avg Meter Price (From)</label>
                        <input type="number" name="average_meter_price_from" value="{{ old('average_meter_price_from', $compound->average_meter_price_from) }}" id="No">
                    ...
                    <div class="form-group">
                        <label for="No">Avg Meter Price (To)</label>
                        <input type="number" name="average_meter_price_to" value="{{ old('average_meter_price_to', $compound->average_meter_price_to) }}" id="No">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="status" class="form-select">
                            <option value="under_construction" {{ $compound->status == 'under_construction' ? 'selected' : '' }}>Under Construction</option>
                            <option value="completed" {{ $compound->status == 'completed' ? 'selected' : '' }}>Completed</option>
                        </select>
                        @error('status')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="unit_types" class="form-control">{{ old('unit_types', $compound->unit_types) }}</textarea>
                        @error('unit_types')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description">{{ old('description', $compound->description) }}</textarea>
                        @error('description')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="about">{{ old('about', $compound->about) }}</textarea>
                        @error('about')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="section1">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <textarea name="facilities">{{ old('facilities', $compound->facilities) }}</textarea>
                        @error('facilities')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="services">{{ old('services', $compound->services) }}</textarea>
                        @error('services')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
        ...
        <div class="button-group">
            <button class="publish" type="submit">Update Compound</button>
