...
<div class="container mt-5">
    ...
    <div class="d-flex justify-content-end align-items-center" style=" margin-block-start: 20px; position: relative;">
        ...
        <form method="GET" action="{{ url()->current() }}" class="d-flex align-items-center" style="inline-size: 100%; max-inline-size: 300px;block-size: 50px; background: #FAFAFA; border-radius: 33.33px; padding-inline-end: 10px;">
            <input type="text" name="type" placeholder="Search a user" class="form-control"
        ...
        <a href="{{route('users.add')}}" style="text-decoration: none;">
            <div style="inline-size: 197px;block-size: 50px; position: relative;">
                ...
                <div style="inset-inline-start: 14px;  inset-block-start: 16.23px; position: absolute; color: black; font-size: 11.69px; font-family: Poppins; font-weight: 400; line-height: 17.54px; word-wrap: break-word">Add new user</div>
        ...
        <table class="table table-borderless align-middle custom-table">
            <thead>
                <tr>
                    ...
                    <th style="inline-size: 10%;">Account Tier</th>
                    <th style="inline-size: 15%;">Last Active</th>
                    ...
                    <tr class="table-row">
                        <td>
                            <div class="d-flex align-items-center">
                                <img src="{{ asset('storage/' . $user->image) }}"  alt="User Image" class="rounded-circle user-avatar">
                                <span>{{ $user->name }}</span>
                            </div>
                        </td>
                        <td>{{ $user->email }}</td>
                        <td>{{ $user->phone_number }}</td>
                        <td class="account-tier">@if ($user->roles->isNotEmpty())
                       ...
                       <td>
                            <button class="btn btn-edit" title="Edit" onclick="window.location.href='{{ route('users.update', ['id'=>$user->id]) }}'">
                                <i class="fas fa-edit"></i>
                            </button>
                        </td>
                        <td>
                            <button class="btn btn-delete" title="Delete" onclick="window.location.href='{{ route('users.delete', ['id'=>$user->id]) }}'">
